...
url2 <- "https://ecoforum.strangeloopgames.com/category/20/modding"
url3 <- "https://ecoforum.strangeloopgames.com/category/24/ideas-feedback"
url4 <- "https://ecoforum.strangeloopgames.com/category/18/game-stories"
url5 <- "https://ecoforum.strangeloopgames.com/category/8/community-help"

urlList <- c(url1, url2, url3, url4, url5)
...
topicGrabber <- function(url){
  ...
  while(loopLive){
    ...
    nextPage <- read_html(paste0(url,"?page=",i))
    nextPageLinks <- nextPage %>% html_nodes('.title') %>% html_nodes(xpath = "a")
...
postGrabber <- function(frameList){
  for(i in 1:length(frameList)){
    ...
    for(j in 1:length(threadList)){
      ...
      threadPage <- read_html(threadLink)
